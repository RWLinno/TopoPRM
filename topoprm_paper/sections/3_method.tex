...
\subsection{TopoSD-Zero: Topology-Aware Self-Distillation}
\label{sec:toposd_zero}
...
to train-test distribution mismatch. To address both, we extend SD-Zero
\citep{sdzero2026} with topology-aware prompts and run it as a second
distillation stage we call \textbf{TopoSD-Zero}.

\paragraph{Phase~III-A: Self-Revision Training (SRT).}
...
The SRT model $\pi_{\mathrm{SRT}}$ is fine-tuned with
\begin{equation}
\label{eq:srt}
\mathcal{L}_{\mathrm{SRT}} =
...
\paragraph{Phase~III-B: On-Policy Self-Distillation (OPSD).}
The SRT model is frozen as a reviser-teacher; the generator-student is the
same architecture with its own trainable parameters $\theta$. For each
problem in $\mathcal{D}_2$ the student samples on-policy
$y\!\sim\!\pi_\theta(\cdot\mid x)$; the frozen reviser-teacher computes the
per-token distribution
$\pi_{\mathrm{SRT}}(\cdot\mid x,y,P_r,y_{<t})$ conditioned on the same
topology-aware $P_r$. The OPSD loss matches student to teacher token-wise
via reverse KL:
\begin{equation}
\label{eq:opsd}
...
\pi_{\mathrm{SRT}}(\cdot\mid x,y,P_r,y_{<t})
\right).
\end{equation}

\paragraph{Why topology-aware $P_r$ helps.}
SD-Zero shows that the reviser diverges maximally from the generator only at
a sparse set of \emph{pivotal} tokens---the KL gradient concentrates there,
giving an implicit token-level credit assignment from a scalar outcome
reward. Our $P_r$ carries not only $r_{\mathrm{out}}$ but also
$r_{\mathrm{topo}}$, $r_{\mathrm{cont}}$, and the specific orphan step index.
This makes the KL signal fire at tokens that are structurally suspect even
when $r_{\mathrm{out}}{=}1$, converting our deterministic DAG diagnostics
into dense per-token supervision without any human annotation.

\paragraph{Student compression ($\leq$4B).}
Because Phase~III-B student rollouts are on-policy and format-closed, we can
distil into a much smaller student without the stopping-failure we observed
with plain RKL distillation. The same OPSD objective is applied with
$\pi_\theta$ initialised from Qwen3.5-4B, Qwen3.5-2B, or Qwen3.5-0.8B;
the teacher remains the 9B SRT model. This is the route we report in
