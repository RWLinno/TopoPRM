#!/usr/bin/env bash
# ============================================================================
# submit_k8s_pytorch.sh
#
# Worker-side entrypoint for a "PyTorch distributed" custom task on a managed
# Kubernetes-based ML platform (MLP). When such a PyTorchJob is submitted (via a
# web console or a vendor CLI), every worker container is started with these env
# vars injected:
...
#
# Submitter (template; run on a machine that has the platform CLI installed).
# ${CLUSTER_SUBMIT_CMD} is a vendor CLI for cluster job submission, and
# ${DOCKER_REGISTRY} is the container registry hosting the task image:
#   ${CLUSTER_SUBMIT_CMD} \
...
echo "[mlp] worker=$WORKER_IDX/$WORKER_NUM host=$(hostname) gpus=$GPUS_PER_WORKER tasks=[$TASK_IDS_FOR_ME]"

if [[ -z "$TASK_IDS_FOR_ME" ]]; then
    echo "[mlp] no tasks assigned to this worker; exiting cleanly."
    exit 0
fi
