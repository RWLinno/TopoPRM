...
Given an input $x=(q,s)$, where $q$ is a mathematical problem and $s$ is optional context, a policy $\pi_\theta$ generates a completion $y$:
\begin{equation}
\label{eq:gen}
...
\subsection{Multi-Source Reward Module}
...
% 这里不能有写死的值，在代码和方法都需要改正这一点。而且它并不是一个重点。
