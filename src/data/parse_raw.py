...
Usage::
    ...
    python -m src.data.parse_raw \\
        --input_dir ${EXP_ROOT}/ms-swift/data/math_raw \\
...
def _parse_infer_line(obj: dict[str, Any]) -> Optional[dict]:
    ...
    # The section markers below are verbatim delimiters emitted by the upstream
    # grading API (the source records are Chinese), so they are matched as-is.
...
def main() -> None:
    ...
    parser.add_argument(
        "--input_dir",
        type=str,
        default=os.path.expandvars("${EXP_ROOT}/ms-swift/data/math_raw"),
