...
\item \textbf{End-to-end training recipe with topology-verified compression.} We instantiate TopoPRM as plug-in reward modules compatible with standard GRPO stacks, and add a \emph{topology-verified self-distillation} stage (TVSD) that reuses the same deterministic diagnostics to densify the scalar outcome signal into per-token supervision, compressing the teacher into compact $\leq$4B students without any external teacher or human step-level annotation.
