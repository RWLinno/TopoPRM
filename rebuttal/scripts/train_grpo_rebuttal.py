#!/usr/bin/env python3
"""GRPO training for rebuttal baselines via TRL (no vLLM dependency).

The installed vLLM (0.20.2 dist-info shadowed by a 0.6.0 stub) is broken and
requires torch 2.11 to reinstall, which would break the working torch 2.5.1
environment. TRL's GRPOTrainer with use_vllm=False generates rollouts with
plain HF transformers, so we use it here for the rebuttal baselines.

Reward is selectable so this one script covers the outcome+length baseline
(--reward outcome_length) and a reproducibility run (--reward topo_hierarchical).

Usage:
  CUDA_VISIBLE_DEVICES=4,5 python rebuttal/scripts/train_grpo_rebuttal.py \
      --reward outcome_length \
      --sft_adapter rebuttal/ckpts/sft-dr1-7b-final \
      --output_dir output/grpo_outcome_length_dr1_7b --max_steps 200
"""
...
DEFAULT_BASE = "/Knowin/foundation/models/deepseek-ai/DeepSeek-R1-Distill-Qwen-7B"
...
def main() -> None:
    ...
    ap.add_argument("--sft_adapter", default="rebuttal/ckpts/sft-dr1-7b-final")
