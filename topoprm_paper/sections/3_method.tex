...
\subsection{Topology-Verified Self-Distillation (TVSD)}
\label{sec:tvsd}

The plain reverse-KL distillation of Eq.~\ref{eq:distill} suffers from two
practical issues when the teacher is a strong RLVR-trained policy: (a) very
few teacher rollouts emit a complete \texttt{<think>$\cdots$<answer>} format
(in our pilot, only $0.4\%$ of 32B teacher traces close the \texttt{<answer>}
tag under limited budget), so the student never sees the stopping pattern;
and (b) the student's own on-policy trajectories are ignored entirely, leading
to train-test distribution mismatch. To resolve both while staying within the
verifiable-reward paradigm, we propose \textbf{Topology-Verified
Self-Distillation (TVSD)}, which upgrades Eq.~\ref{eq:distill} to an
on-policy setting whose supervision is densified through the same deterministic
topology / continuity diagnostics used in Eq.~\ref{eq:r_topo}~and~\ref{eq:r_cont}.
TVSD runs in two short phases on top of the Stage~II teacher.
...
where the process signal is converted into an in-context teacher.
...
Because Phase~III-B student rollouts are on-policy and format-closed, we can
compress into a much smaller student without the stopping failure we observed
for the legacy reverse-KL student in Eq.~\ref{eq:distill}. The same TVSD
objective is applied with $\pi_\theta$ initialised from Qwen3.5-4B,
Qwen3.5-2B, or Qwen3.5-0.8B; the teacher remains the 9B refined model.
This is the route we report in Section~\ref{sec:experiments}.
