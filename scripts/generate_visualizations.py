...
def plot_sft_training_curves() -> None:
    ...
    ax.set_title("SFT token accuracy curve")
    ax.grid(True, alpha=0.3)
    ax.annotate(
        f"{accs[0]:.3f}",
        (steps[0], accs[0]),
...
def plot_data_statistics() -> None:
    ...
    # Category bar chart
    ax = axes[0]
    # Prefer the canonical category order; if the dataset uses different
    # category labels, fall back to the most frequent ones observed in the data.
    cats = [c for c in CATEGORY_ORDER if c in categories]
    if not cats:
        cats = [c for c, _ in categories.most_common(len(CATEGORY_COLORS))]
    counts = [categories.get(c, 0) for c in cats]
    bars = ax.bar(
        cats, counts, color=CATEGORY_COLORS[: len(cats)], edgecolor="white", linewidth=0.8
    )
    ...
    ax.set_title(f"Data category distribution ({sum(counts)} records)")
    ...
    ax.set_xlabel("Procedure score (procedure_score)")
    ax.set_ylabel("Frequency")
    ax.set_title(
        f"Score distribution (mean={np.mean(scores):.3f}, median={np.median(scores):.3f})"
    )
    ax.grid(axis="y", alpha=0.3)
    ax.axvline(np.mean(scores), color="#d62728", linestyle="--", linewidth=1, label="mean")
    ax.legend()
...
def plot_reward_weights() -> None:
    ...
    ax.set_title("TopoPRM reward component weights")
