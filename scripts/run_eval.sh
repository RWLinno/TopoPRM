...
swift infer \
    --model "$MODEL" \
    --adapters "$ADAPTER" \
    --val_dataset "data/test/infer_middle_school_full_2181.json" \
    --max_new_tokens 2048 \
    --temperature 0.1 \
    --infer_backend vllm \
...
swift infer \
    --model "$MODEL" \
    --adapters "$ADAPTER" \
    --val_dataset "data/test/infer_high_school_full_5414.json" \
    --max_new_tokens 2048 \
    --temperature 0.1 \
    --infer_backend vllm \
    --vllm_gpu_memory_utilization 0.9 \
...
python3 -m src.eval.critique_eval \
    ...
    --ground_truth "data/test/infer_middle_school_full_2181.json" \
    --output "output/eval/${OUTPUT_NAME}_middle_metrics.json"

python3 -m src.eval.critique_eval \
    --predictions "output/eval/${OUTPUT_NAME}_high.jsonl" \
    --ground_truth "data/test/infer_high_school_full_5414.json" \
