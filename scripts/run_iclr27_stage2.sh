...
case "$VARIANT" in
  ...
  sarl)
    ...
    export SARL_EMBED_MODEL="${SARL_EMBED_MODEL:-${MODEL_ROOT}/Qwen3-Embedding-0.6B}"
...
# Direct safetensors writes to ${EXP_ROOT} reproducibly produced zero-filled
# files. Train on the writable demo filesystem, then archive verified artifacts.
export GRPO_OUTPUT_DIR="${STAGE2_OUTPUT_DIR:-${EXP_ROOT}/canonical/grpo_${OUTPUT_TAG}_qwen35_9b}"
export GUARD_GPU_LEAK_MB="${GUARD_GPU_LEAK_MB:-4096}"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3}"
export NPROC_PER_NODE="${NPROC_PER_NODE:-4}"
export AUTO_RESOLVE_SFT_ADAPTER=0
export TOPOPRM_ENV_BIN="${TOPOPRM_ENV_BIN:-${PYTHON_ENV_BIN}}"

STAGE1_ROOT="${EXP_ROOT}/canonical/sft_qwen35_9b"
