...
# Dataset cache location: honour HF_DATASETS_CACHE, then HF_HOME, then the
# standard per-user cache. No machine-specific path is baked in.
HF_DATASETS_CACHE_DIR = os.environ.get(
    "HF_DATASETS_CACHE",
    os.path.join(
...
def extract_mcq(text: str) -> str | None:
    ...
    cited inside reasoning when <answer> is absent), tracked in the
    experiment resync notes.
    """
...
def load_livecode() -> list[dict]:
    """LiveCode is permanently disabled.

    Our TopoPRM models are trained purely for math reasoning. Empirical
    validation (see the experiment roadmap notes) confirmed that every
...
def main():
    ...
    parser.add_argument("--allow_mmlu_sft_style", action="store_true",
                        help="Override the safety skip for MMLU when sft_style=True. "
                             ...
                             "the experiment resync notes for the open TODO.")
