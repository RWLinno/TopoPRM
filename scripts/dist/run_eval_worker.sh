...
#        - 1st CLI arg ($1)
#        - $TASK_ID
#        - $SLURM_ARRAY_TASK_ID   (Aliyun Slurm)
#        - $RAY_TASK_INDEX        (Ray submit_ray.py)
#        - $MLP_ROLE_INDEX        (Volcengine MLP PyTorch distributed task)
...
#
# This script is the single point of contract for every cloud launcher
# (local / Slurm / Volcengine PyTorch DDP / Ray). Per-cloud submitters only
...
MODEL_PATH="${MODEL_PATH:-/Knowin/foundation/weilinruan/hf_models/deepseek-ai/DeepSeek-R1-Distill-Qwen-7B}"
...
PYTHON_BIN="${PYTHON_BIN:-/Knowin/foundation/weilinruan/env/topoprm/bin/python}"
